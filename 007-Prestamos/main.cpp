...
void menuPrestamos(Database& db) {
    int opcion;
    do {
        ...
        switch (opcion) {
            case 1: {
                ...
                std::string fp, fd;
                std::cout << "ID Libro: ";              std::cin >> idLibro;
                std::cout << "ID Usuario: ";            std::cin >> idUsuario;
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                std::cout << "Fecha préstamo (YYYY-MM-DD): ";
                std::getline(std::cin, fp);
                std::cout << "Fecha devolución (YYYY-MM-DD): ";
                std::getline(std::cin, fd);
                if (Prestamo::crear(db, idLibro, idUsuario, fp, fd))
                    std::cout << "Préstamo creado correctamente.\n";
                else
                    std::cout << "Error al crear préstamo.\n";
                break;
            }
            case 2: {
                auto lista = Prestamo::listar(db);
                std::cout << "ID\tLibroID\tUsuarioID\tF.Prestamo\tF.Devolución\tF.RealDevolución\n";
            ...
            case 3: {
                int id, idLibro, idUsuario;
                std::string fp, fd, fr;
                std::cout << "ID del préstamo a actualizar: "; std::cin >> id;
                std::cout << "Nuevo ID Libro: ";               std::cin >> idLibro;
                std::cout << "Nuevo ID Usuario: ";             std::cin >> idUsuario;
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                std::cout << "Nueva Fecha préstamo (YYYY-MM-DD): ";
                std::getline(std::cin, fp);
                std::cout << "Nueva Fecha devolución (YYYY-MM-DD): ";
                std::getline(std::cin, fd);
                std::cout << "Fecha real devolución (YYYY-MM-DD o vacío): ";
                std::getline(std::cin, fr);
                if (Prestamo::actualizar(db, id, idLibro, idUsuario, fp, fd, fr))
                    std::cout << "Préstamo actualizado correctamente.\n";
                else
                    std::cout << "Error al actualizar préstamo.\n";
                break;
            }
            case 4: {
                int id;
                std::cout << "ID del préstamo a eliminar: "; std::cin >> id;
                if (Prestamo::eliminar(db, id))
                    std::cout << "Préstamo eliminado correctamente.\n";
                else
                    std::cout << "Error al eliminar préstamo.\n";
                break;
            }
            case 0:
